...
**Figures.** The reconstruction slices below make the mechanism visible: at the
stress point (n_obs = 80, noise = 0.20) the hard-monotone field is a clean monotone
ramp that stays in range, while the vanilla and soft baselines overshoot and
oscillate — dipping *below zero* on the low plateau, an unphysical density (LWR) /
value (Burgers). The crossover curves quantify it: comparable at zero noise, the
hard-monotone L¹ separates cleanly below both baselines as noise grows.

![LWR reconstruction at the stress point](figures/reconstruction-lwr.png)

![Burgers reconstruction at the stress point](figures/reconstruction-burgers.png)

![LWR whole-field L¹ vs noise](figures/crossover-lwr-l1.png)

![Burgers whole-field L¹ vs noise](figures/crossover-burgers-l1.png)

The front-vs-whole-field trade-off is visible in the paired L² panels
(`figures/crossover-{lwr,burgers}-l2.png`): the LWR L² tracks L¹, while the Burgers
L² is a wash at high noise. Admissibility stays flat at zero for the hard-monotone
field on both PDEs while the baselines climb (`figures/admissibility-{lwr,burgers}.png`).
All eight are regenerated by `python -m applications.pinn.experiments.figures`,
which writes each in both **vector PDF** (for the LaTeX build) and PNG (for this
preview); the figures below embed the PNGs.

*(The raw out-of-range fraction was uninformative — dominated by sub-0.01
excursions; a physical-bounds `[0, ρ_max]` metric is the right future measure.)*
