...
"""Smoke tests for the JAX PINN trainer."""
...
def test_training_reduces_loss() -> None:
    """A few optax steps reduce the PINN loss (residual autodiff works)."""
    problem, data = _burgers_forward_data()
    model = builders.build_jax(problem, CFG, "hard_monotone")
    _, history = jax_trainer.train(
...
def test_vanilla_also_trains() -> None:
    ...
    problem, data = _burgers_forward_data()
    model = builders.build_jax(problem, CFG, "vanilla")
    _, history = jax_trainer.train(
        problem, model, data, weights=LossWeights(), sign_x=-1, lr=3e-3, steps=40
    )
    assert history[-1] < history[0]
