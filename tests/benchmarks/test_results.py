...
def test_count_collapses_classification_and_regression() -> None:
    ...
    # classification: base rate 0.79; seeds at/below 0.81 count as collapsed.
    vals = (0.21, 0.21, 0.83, 0.84, 0.84)
    assert (
        _count_collapses(
            vals, task="binary_classification", base_rate=0.79, lower_is_better=False
        )
        == 2
    )
    ...
    assert (
        _count_collapses(reg, task="regression", base_rate=0.0, lower_is_better=True)
        == 1
    )
