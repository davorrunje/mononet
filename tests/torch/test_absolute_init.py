...
def test_absolute_default_weight_scale_and_bias() -> None:
    torch.manual_seed(0)
    in_f, units = 256, 512
    layer = MonoLinear(in_f, units, mode="mixed", activation="elu")
...
def test_absolute_bias_nonzero_off_half() -> None:
    torch.manual_seed(0)
    layer = MonoLinear(64, 64, mode="mixed", activation="elu", convex_fraction=0.25)
...
def test_explicit_init_overrides_absolute() -> None:
    torch.manual_seed(0)
    layer = MonoLinear(64, 64, mode="mixed", activation="elu", init="he_normal")
    assert layer.bias is not None
...
def test_switch_default_unchanged() -> None:
    torch.manual_seed(0)
    layer = MonoLinear(64, 64, mode="split", activation="elu")
    assert layer.bias is not None
    assert torch.allclose(layer.bias.detach(), torch.zeros(64, dtype=layer.bias.dtype))
