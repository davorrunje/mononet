...
`defendable-science` ships **no template** for the design/plan stage of a hypothesis. This
...
`implement`, bound per project in `.defendable-science/config.yml` as `engineering_backend:`).
...
- `defendable-science` does not design experiments, write plans, or implement — it composes
