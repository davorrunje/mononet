...
def _import_submodules(
    ...
    if include_public_api_only:
        ...
        # Extract only mononet/__init__.py or mononet/<something>/__init__.py
        public_api_modules = [
            m
            for m in modules
...
) -> list[tuple[str, FunctionType | type[Any]]]:
    funcs_and_classes = []
...
def _import_all_members(
    ...
    members: list[tuple[str, FunctionType | type[Any]]] = list(
        itertools.chain(
            *[
...
def _update_single_api_doc(
    symbol: FunctionType | type[Any], docs_path: Path, module_name: str
) -> None:
...
def _update_api_docs(
    symbols: list[FunctionType | type[Any]], docs_path: Path, module_name: str
) -> None:
